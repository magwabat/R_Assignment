...
```
# Use full path for creating directories
...
```
#Visualizing SNP diversity on each chromosome # is this the same as number of SNP per chromosome?
```{r}
SNP_Heterozygosity <- 
...
dev.off()
```
# Also include zygosity for each group
#Personal trial (Please first ignore this. It is still under process) # Ok remember to finish off before deadline
##Preparing files
```{r}

...
dim(fang)#gives the dimensions of the file
dim(SNPs)
...
for (i in 1:length(unique(chr_maize$Chromosome))) {
  ...
  write.table (chrom, file = paste("./Maize_info/Maize_incr", i, sep = "_"), quote = FALSE)
}
```
...
for (i in 1:length(unique(Chrom_Decr_Maize$Chromosome))) {
  ...
  write.table(chrm_decreasing, file = paste("./Maize_info/Maize_decr", i, sep = "_"), quote = FALSE)
}
```
...
for (i in 1:length(unique(chr_teo$Chromosome))) {
  ...
  write.table (Tchrom, file = paste("./Teosinte_info/Teosinte_incr", i, sep = "_"), quote = FALSE)
}
```
...
for (i in 1:length(unique(Tchrom$Chromosome))) {
  ...
  write.table(Tchrm_decreasing, file = paste("./Teosinte_info/Teosinte_decr", i, sep = "_"), quote = FALSE)
}
```
#Data visualization
...
```
#Distribution of SNPs across each chromosome (SNP density)
```{r}
Distr_across_chr <- 
...
```
#My Own piece
I tried to visualize how the density of SNPs associated with homozygous A varries in the Fang_et_algenotypes data.
```{r}
new_fang <- fang[-(1:3)]
...
Density_SNP_for_AA <- ggplot(data = single_file) + geom_density(mapping = aes(x=Total_A), fill="red")
